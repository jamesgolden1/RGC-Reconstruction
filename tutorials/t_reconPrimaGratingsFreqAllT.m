...
% dirname1 = 'prosthesis_70_gratings_aug16_cont';
% dirname2 = 'prosthesis_70_gratings_aug16_nocont';% freqArr = [.05 .5 1 1.5 2 2.5 3 3.5 4 4.5 5 5.5 6 8 12 25];

dirname1 = 'prosthesis_70_gratings_aug25_cont';

dirname2 = 'prosthesis_70_gratings_aug25_nocont'; freqArr = [.5 1 1.5 2 2.5 3 3.5 4 4.5 5 5.5 6 6.5 7 7.5 12];
numReps =2;

maxTrial = 500;%size(framesOut,3);
...
load(fullfile(['/Users/james/Documents/MATLAB/RGC-Reconstruction/dat/' dirname1],[sprintf('primaRecon_freqH%4d_cont%6d.mat',100*freqArr(freqInd),1e5*contrastArr(contrastInd))]),'trialReconPlay','contrast','spikeTrials');
...
% spikeOn = spikeTrials;
jj = 8;
spikeOnFull = spikeTrials((jj-1)*9717+1:jj*9717,:);
spikeOnAll = zeros(size(spikeOnFull));
spikeOnAll(dropoutIndices,:) = spikeOnFull(dropoutIndices,:);
spikeOn = zeros(size(spikeOnAll));
spikeOn(1:1+28*32,:) = spikeOnAll(1:1+28*32,:);
spikeOn(28*32+31*35+1:28*32+31*35+55*63+1,:) = spikeOnAll(28*32+31*35+1:28*32+31*35+55*63+1,:);

trialReconPlayXW = filterMat2'*spikeOn;

% trialReconPlayXW = zeros(10000,500);
% for jj = 7:8
%     
% %     trialReconPlayXW((jj-1)*10000+1:jj*10000,:) = filterMat2'*spikeOn((jj-1)*9717+1:jj*9717,:)
% %     trialReconPlayXW = filterMat2'*spikeOn((jj-1)*9717+1:jj*9717,:);
% 
%     spikeOnAll = spikeTrials((jj-1)*9717+1:jj*9717,:);
%     spikeOn = zeros(size(spikeOnAll));
%     spikeOn(1:1+28*32,:) = spikeOnAll(1:1+28*32,:);
%     spikeOn(28*32+31*35+1:28*32+31*35+55*63+1,:) = spikeOnAll(28*32+31*35+1:28*32+31*35+55*63+1,:);
%     trialReconPlayXW = trialReconPlayXW + filterMat2'*spikeOn;
% end
...
% subplot(122); imagesc(reshape(dataRaw(:,500+10),[100 100])); colormap gray;
%%
...
% load('ws_gratings_primaOnlyOn_aug25_2018_8reps_sh8_f10_out0.mat');

%     load('ws_gratings_primaLearning_aug25_2018_8reps_out0.mat'); 
...
hold on;
for radVal = 1%1:15
    plot(freqArrSort(:),Pbigsort(sortInd),'-bo','linewidth',4);
end
