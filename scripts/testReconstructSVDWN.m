...
elseif strcmp(name,'wn')
%     matfOn = matfile('../dat/spikeResp_may26_on.mat');
%     matfOff = matfile('../dat/spikeResp_may26_off.mat');
% load('WNstim_response_OffParasol_wntest.mat')
% load('WNstim_response_OnParasol_wntest.mat')
...
if strcmp(name,'grating') || strcmp(name,'bar')    
    blocklength = 152;    
    frstart = 1;
    frend = frstart + blocklength-1;
    spikesout = double(matfOff.spikesoutsm);    
    stim = double(reshape(matfOff.whiteNoiseSmall,96*96,blocklength));
    spikeRespOff = downSampResp(spikesout(:,frstart:1*frend), numcells, blocklength);

elseif strcmp(name,'wn')
%     spikesoutbig = (matfOff.spikeResp);
%     spikesout = spikesoutbig(:,1:1*blocklength);
%     clear spikesoutbig
%     load ../dat/movie_may26.mat
%     load('WNstim_response_OffParasol_wntest.mat')
%     spikesout = double(spikesoutsm);    
%     stim = double(reshape(whiteNoiseSmall,96*96,blocklength));

    matfOff = matfile('../dat/spikeResp_may26_off.mat');
    spikeRespOff = matfOff.spikeResp(:,frstart:1*frend);%downSampResp(matfOFF.spikeResp(:,frstart:1*frend), numcells, blocklength);
...
if strcmp(name,'grating') || strcmp(name,'bar')  
    ...
    frend = frstart + blocklength-1;
    spikesout = double(matfOn.spikesoutsm);
    stim = double(reshape(matfOn.whiteNoiseSmall,96*96,blocklength));
    spikeRespOn = downSampResp(spikesout(:,frstart:1*frend), numcells, blocklength);

elseif strcmp(name,'wn')
%     spikesoutbig = double(matfOn.spikeResp);
%     spikesout = spikesoutbig(:,1:1*blocklength);
%     clear spikesoutbig
%     load ../dat/movie_may26.mat
% load('WNstim_response_OnParasol_wntest.mat')
...
%     stim = double(reshape(whiteNoiseSmall,96*96,blocklength));
    ...
    matfOn= matfile('../dat/spikeResp_may26_on.mat');
    spikeRespOn = matfOn.spikeResp(:,frstart:1*frend);
...
for ishift = 0%:5;
% barmov = matfOn.whiteNoiseSmall;
barmov = reshape(stim(:,1:blocklength-30),[96 96 blocklength-30]);
barmovShort = double(barmov(:,:,ishift+[1:blocklength-30]));%/2+.5;
...
%     movrecons_on_off = movrecons_on_off_full;
    movrecons_on_off_rz = .5*movrecons_on_off_full./mean(movrecons_on_off_full(:));
...
%     movrecons_on_off(movrecons_on_off_full>0.5) = 1; movrecons_on_off(movrecons_on_off_full<0.5) = 0;
    ...
    movrecons_on_off(movrecons_on_off_rz>0.5) = 1; movrecons_on_off(movrecons_on_off_rz<0.5) = 0;
...
%     figure; hist(abs(err1(:)),120); skew(abs(err1(:)))
