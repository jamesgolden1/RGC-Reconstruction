...
%     tight_subplot(4,4,pltCtr);
    ...
    staex = stawn(:,paraIndPlus); staim = reshape(staex,[100 100]); stamax = max(abs(staex(:)));
...
%     title(sprintf([titleStr{mind} '\nCell %d WN STA'], paraInd),'fontsize',14);
%     if mod(mindCtr,2)
...
%     tight_subplot(4,4,pltCtr); 
    axes(ha(pltCtr));
    pltCtr = pltCtr+1;
    filtIm = reshape(filterWN(1+paraIndPlus,:),[100 100]);
    imagesc(signVal(mind)*filtIm);
    mabs = max((filtIm(:)));
    minabs = min((filtIm(:)));
    caxis([minabs 1*mabs]); colormap gray;
    axis image;
%     title(sprintf([titleStr{mind} '\nCell %d WN Decoding'], paraInd),'fontsize',14); % set(gca,'fontsize',14);
%     if mod(mindCtr,2)
    axis([fmaxr(paraIndPlus+1)-windsize , fmaxr(paraIndPlus+1)+windsize,fmaxc(paraIndPlus+1)-windsize , fmaxc(paraIndPlus+1)+windsize]);
%     end
...
%     tight_subplot(4,4,pltCtr); 
    axes(ha(pltCtr));
    pltCtr = pltCtr+1;
    staex = stans(:,paraIndPlus); staim = reshape(staex,[100 100]); 
    stamax = max(abs(staex(:))); 
%     stamin = min((staex(:)));
    imagesc(signValRF(mind)*staim.*(.1+.9*reshape(exp(-.005*dp(1+paraIndPlus,:).^2),[100 100])));
%     caxis([-stamax stamax]);


    if mosind == 1 || mosind == 3
    caxis([-stamax -.25*stamax]);
    else
    caxis([.25*stamax stamax]);
    end
    axis image; colormap gray;
%     title(sprintf([titleStr{mind} '\nCell %d NS STA'], paraInd),'fontsize',14);
%     if mod(mindCtr,2)
    axis([fmaxr(paraIndPlus+1)-windsize , fmaxr(paraIndPlus+1)+windsize,fmaxc(paraIndPlus+1)-windsize , fmaxc(paraIndPlus+1)+windsize]);
%     end
    axis off
...
savefig(['/Volumes/Lab/Users/james/current/RGC-Reconstruction/dat/filters/healthy_all.fig']);
