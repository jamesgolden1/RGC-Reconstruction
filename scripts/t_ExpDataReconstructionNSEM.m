...
for nameInd = 1:length(names)
    ...
    for blockNum = 2:2:118
        ...
        cellSpikes{nameInd} = [cellSpikes{nameInd}; (blockNumCtr-1)*7200*.00832750+cellSpikesTemp];
    end
end
%% Get STA
...
spikemat = zeros(length(cellSpikes),430000);
zshift =0;
for cellind = 1:length(cellSpikes)
...
spikematzm = spikemat - spikematmean*ones(1,430000);
% figure; imagesc(spikematzm*spikematzm');
spikematnorm = spikematzm./((std(spikematzm')'*ones(1,430000)));
...
for evInd = length(evArr)
    for trainFractionInd = length(trainFraction)
        ...
        if pRecon.windowSize == 1
            filterFile = ['nsem_2013_08_19_6' cell_type '_sh' sprintf('%d',zshift) '_sv' sprintf('%2d',100*evArr(evInd)) '_tr' sprintf('%2d',100*trainFraction(trainFractionInd)) mosaicFile];
        else
            filterFile = ['nsem_2013_08_19_6/' cell_type '_sh' sprintf('%d',zshift) '_sv' sprintf('%2d',100*evArr(evInd)) '_tr' sprintf('%2d',100*trainFraction(trainFractionInd)) '_wind' sprintf('%d',pRecon.windowSize) mosaicFile];
        end
